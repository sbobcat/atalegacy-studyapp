...
function parseXlsx(filePath: string): RawRecord[] {
  ...
  const sheetName =
    workbook.SheetNames.find(
      (name) =>
        name.toLowerCase().includes("question bank") ||
        name.toLowerCase().includes("question_bank"),
  ...
  if (headerRowIndex === -1) {
    emitError(
      'Could not find a row with "Question_ID" column header in the spreadsheet.',
    );
    return [];
  }

  // Re-parse using the detected header row
  const headerRow = (rawArrayRows[headerRowIndex] as string[]).map((h) =>
    String(h).trim(),
  );
...
function checkObjectiveChoices(rec: RawRecord): boolean {
  ...
  const answerLabels = rec.Correct_Answer.split(/[,;]/)
...
function run(): void {
  ...
  if (validatedRecords.length === 0) {
    emitError(
      "Output contains no questions after all filters and validations.",
    );
  }
  ...
  if (dryRun) {
    console.log("Dry run complete; validated output was not written.");
    return;
  }
